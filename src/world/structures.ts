...
export function pad(
  ...
  for (let z = rect.z1 - (edgeRamp ? 4 : 0); z <= rect.z2 + (edgeRamp ? 4 : 0); z++) {
    for (let x = rect.x1 - (edgeRamp ? 4 : 0); x <= rect.x2 + (edgeRamp ? 4 : 0); x++) {
      ...
      // If the column was previously void, build a solid pillar up to the pad so
      // landmarks never float above an empty gap or sink into nothing.
      ...
      // Solid foundation under the pad top so nothing is left hovering.
