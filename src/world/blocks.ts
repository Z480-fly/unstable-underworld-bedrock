...
// ---------------------------------------------------------------------------
// Helpers for blocks whose Bedrock states are easy to get wrong.
// ---------------------------------------------------------------------------
...
/** Bedrock slabs: single `top_slot_bit` for top slabs. */
export function slab(name: string, top = false): BlockState {
  return bs(name, { top_slot_bit: top });
}

/** Bedrock walls connect through the four per-side connection states. */
...
export const P = {
  ...
  // --- masonry --------------------------------------------------------------
  // Bedrock 1.26 flattened these IDs to the Java-style names (verified against
  // minecraft-data bedrock_1.26.51, which is the target version).
  ...
  // --- woods (village, fields, keep interiors) ------------------------------
